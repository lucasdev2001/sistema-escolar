...
const Home = ({data})=>{
  ...
  <div className="offcanvas offcanvas-bottom h-75 p-5" tabIndex="-1" id="offcanvasBottom" aria-labelledby="offcanvasBottomLabel">
  ...
  <form onSubmit={handleSubmit}>
        <div className="mb-3">
            ...
            <input type="text" className="form-control" name='nome' onInput={handleChange} required/>
        </div>
        <div className="mb-3">
            <label className="form-label">CPF</label>
            <input type="text" className="form-control" minLength="14" name='cpf' id='cpf' onInput={handleChange} autoComplete="none" required />
        </div>
        <div className="mb-3">
        <label className="form-label">Sexo</label>
        <div className="form-check">
        <input className="form-check-input" type="radio" name="sexo" id="sexo" value="masculino" onInput={handleChange} required />
        ...
        <input className="form-check-input" type="radio" name="sexo" id="sexo" value="feminino" onInput={handleChange}  required />
        
        </div>
        </div>
        <div className="mb-3">
            <label className="form-label">Email</label>
            <input type="email" className="form-control" name='email' id='email' onInput={handleChange} required/>
        </div>
        <div className="mb-3">
            <label className="form-label">Telefone</label>
            <input type="text" className="form-control" minLength="15" name='telefone' id='telefone'  onInput={handleChange} autoComplete="none" required/>
        </div>
        <p>{message}</p>
        <div className='row g-2'>
            <div className='col-12'><button type="submit" className="btn btn-primary ml-5">Cadastrar</button></div>
      ...
      <div>
        ...
        <div className="col-12"><button type="button" className="btn btn-dark mb-3 mt-5" data-bs-toggle="modal" data-bs-target="#modalTabela">Em tabela</button></div>
        <div className="col-12" hidden id="spinner"><div className="spinner-border text-success" role="status">
        <span className="visually-hidden">Loading...</span>
        </div>
        </div>
        </div>
    ...
    <div className="container-fluid text-center">
      ...
      <div className="row row-cols-1 row-cols-md-3 g-5 d-flex justify-content-center">
        {data.map((data)=>{
          return(
            <Card key={data._id} id={data._id} nome={data.nome} cpf={data.cpf} sexo={data.sexo} email={data.email} telefone={data.telefone}/>
          )
        })}
      </div>
...
<div className="modal fade" id="modalTabela" tabIndex="-1" aria-labelledby="modalTabelaLabel" aria-hidden="true">
    <div className="modal-dialog modal-fullscreen">
      <div className="modal-content">
        <div className="modal-header">
          <h5 className="modal-title" id="modalTabelaLabel">Tabela de registros</h5>
